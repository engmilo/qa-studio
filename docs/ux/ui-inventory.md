...
\- `tipGenerator`, `tipDashboard`, `tipProjects`, `tipSuites`, `tipHistory`,
...
\- \*\*Import / Export Data\*\* (see §8.1)

\- \*\*Clear All Data\*\* confirmation (see §8.2)
...
Marketing/intro page. Sells the app and routes users into `app.html`.
...
&#x20; ("Paste or type your user stories below — copy them directly from Jira,

&#x20; Notion, spreadsheets, or any requirements document.")
...
&#x20; - \*\*Save to Project\*\* — `saveProject` → opens Save modal (§8.3)

\- \*\*Results table\*\* columns:

&#x20; - `colId`, `colPriority`, `colStatus`, `colTitle`
...
\- Click \*\*Generate\*\* → POST to Worker (`generateTestCases`)
...
&#x20; - `qa\_usage\_total` (increment by result count)

&#x20; - `qa\_history` (push new entry)

&#x20; - `qa\_projects` (only on Save)
...
&#x20; so renderer must allow trusted HTML for that specific key. Document the

&#x20; rule in the i18n module to prevent accidental XSS later.
...
\- Message: `dashEmpty` — "No test cases yet. Generate some to see stats here."
...
\- \*\*Charts\*\* (built without a chart library — TBC, confirm during Phase 4)
...
&#x20; - `sideAutoCoverage` (`sideOfTotal` — "of total executed")



\### User actions

\- (Mostly read-only.) Possibly drill-down click-throughs to Projects (TBC).



\### Data sources

\- \*\*Reads:\*\* `qa\_history`, `qa\_projects`, `qa\_usage\_total`
...
\- \*\*Writes:\*\* `qa\_projects` (delete project, mutate cases)
...
&#x20; schema in \[`storage-model.md`](../database/storage-model.md#32-project).
...
\- \*\*Table\*\* columns: `colFeature`, `colGenerated` (date), `colTime`, `colCount`
...
\- Confirm whether "Clear History" requires a confirmation modal. If not

&#x20; today, consider adding one — accidental wipe is a likely papercut.
...
\- Triggered by `importExportBtn` ("Import / Export")

\- Title: `importExportTitle`

\- Two sections:

&#x20; - \*\*Export\*\* — `exportLabel`, button `downloadBackup` ("Download JSON Backup")

&#x20; - \*\*Import\*\* — `importLabel`, button `selectJsonFile` ("Select JSON File")

\- Warning: `importWarning` — "Warning: Importing will replace all existing data."
...
\[`storage-model.md`](../database/storage-model.md#9-import--export-compatibility).
...
\- Confirmation: `confirmClearAll` — "Clear ALL data (projects, history, test

&#x20; cases)? This cannot be undone."
...
&#x20; - Project name — placeholder `projectNamePlaceholder` ("Project name…")

&#x20; - Description — placeholder `projectDescPlaceholder` ("Description
...
\- On success toast: `deletedTests` — "Deleted selected test cases"
...
\- Content (TBC) — likely a way to override `WORKER\_URL` or model id for

&#x20; power users. Confirm during Phase 1 since this view directly touches
...
\### 10.1 Search inputs (consistent pattern)

Three views ship a search box with slightly different placeholders:
...
Used in Generator and Projects detail. Identical mechanics in both. Strong

candidate for a shared component in `src/ui/` (kept small — no over-engineering).



\### 10.3 Pluralization

The codebase already has function-valued i18n entries:
...
This is \*\*why locales are `.js`, not `.json`\*\*, per

\[`target-architecture.md §2`](../architecture/target-architecture.md#2-target-tree).



\### 10.4 Success affordance — confetti

`libs/confetti.browser.min.js` is loaded for a reason. Confirm exact trigger

(generation success? first save?) during Phase 4 and wire it through

`src/ui/confetti.js` rather than view-level direct calls.
...
Changes `qa\_lang`, re-renders every label. The refactor's `setLang(lang)`

in `i18n/index.js` must invoke a re-render via `store.subscribe`, not by
...
\- \[ ] Color is not the only signal for status (Pass/Fail/Blocked/Untested)
...
> A dedicated a11y audit can land in `docs/ux/accessibility.md` later.
...
\- Mobile behavior of tables / dashboard charts is \*\*TBC\*\* — verify during

&#x20; Phase 4 and capture screenshots before splitting views.
...
\## 13. What's Not in the UI Today (Useful Negative Space)



Documenting what's intentionally absent prevents accidental scope creep:



\- No user accounts, login, or sign-up flow.

\- No collaboration / multi-user features.

\- No comments, attachments, or rich-text editing on test cases.

\- No issue-tracker integrations (Jira/Linear/etc.) — Jira is referenced only

&#x20; as a \*source\* the user pastes from.

\- No analytics dashboard for the operator (Worker logs live in Cloudflare).

\- No notifications (push or email).



If any of those becomes a future ask, it gets its own design doc — not a
...
\- \[ ] Does API Configuration (`apiConfig`) actually let users override

&#x20;     `WORKER\_URL` / model, or is it informational?

\- \[ ] Where does the \*\*Demo\*\* entry (`demo`) live and what does it do?

\- \[ ] Does the Suites view have its own empty-state key or reuse another?

\- \[ ] Are tags persisted per `TestCase` or computed at render time?

&#x20;     (Also open in `storage-model.md`.)
