...
Caches the static shell. Must \*\*not\*\* cache Worker responses (see security doc).
...
Out-of-repo (or sibling repo — see open question in worker-proxy.md).

Holds the AI provider key; enforces rate limits and origin allow-listing.
...
\[localStorage updated: qa\_history, qa\_projects, qa\_usage\_total]
...
| i18n | inline dictionary in `app.js` | `src/i18n/locales/\*.json` |

| State / storage | direct `localStorage` calls scattered in `app.js` | `src/state/store.js` + `src/state/storage.js` (with schema versioning) |
...
3\. \[`architecture/target-architecture.md`](target-architecture.md) — \*(planned)\* end-state design
...
7\. \[`ux/ui-inventory.md`](../ux/ui-inventory.md) — \*(planned)\* every screen + state
...
\## 9. Design Principles (carry these into every PR)
...
2\. \*\*No build step we don't need.\*\* Native ES modules first; reach for a

&#x20;  bundler only when there's a concrete reason.
...
\- \*\*File encoding:\*\* UTF-8 (no BOM). Enforced by future `.editorconfig` +

&#x20; CI grep (see `known-issues.md` ISSUE-001).
...
\*\*Next code work:\*\* Phase 0 of `roadmap/refactor-plan.md` (safety net + the

two pre-refactor cleanups).
