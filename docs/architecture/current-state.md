...
| `manifest.json` | PWA manifest (`start_url: /qa-studio/app.html`) |
...
1. **CONFIG** — `WORKER_URL` constant pointing at the Cloudflare proxy.
...
| Key | Type | Purpose |
|---|---|---|
| `qa_lang` | string | Selected UI language (default `en`) |
| `qa_usage_total` | int (string) | All-time generated counter |
| `qa_history` | JSON array | Past generation runs |
| `qa_projects` | JSON array | Saved projects (incl. test cases) |
